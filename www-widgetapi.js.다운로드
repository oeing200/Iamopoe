...
"msg="+f.i.length+"&cb="+f.h.length;e.params["params.serviceWorker"]="false";B.document&&B.document.querySelectorAll&&(e.params["params.fscripts"]=String(document.querySelectorAll("script:not([nonce])").length));window.yterr&&"function"===typeof window.yterr&&window.yterr(e);f=0===e.sampleWeight;if(!Ch.has(e.message)&&!f){"ERROR"===d?(wh.J("handleError",e),S("record_app_crashed_web")&&0===Fh&&1===e.sampleWeight&&(Fh++,rh("appCrashed",{appCrashType:"APP_CRASH_TYPE_BREAKPAD"})),Eh++):"WARNING"===d&&
wh.J("handleWarning",e);if(S("kevlar_gel_error_routing")){h=d;g=e;a:{f=u(Hh);for(m=f.next();!m.done;m=f.next())if((n=I)&&0<=n.toLowerCase().indexOf(m.value.toLowerCase())){f=!0;break a}f=!1}if(!f){m={stackTrace:g.stack};g.fileName&&(m.filename=g.fileName);f=g.lineNumber&&g.lineNumber.split?g.lineNumber.split(":"):[];0!==f.length&&(1!==f.length||isNaN(Number(f[0]))?2!==f.length||isNaN(Number(f[0]))||isNaN(Number(f[1]))||(m.lineNumber=Number(f[0]),m.columnNumber=Number(f[1])):m.lineNumber=Number(f[0]));
f={level:"ERROR_LEVEL_UNKNOWN",message:g.message,errorClassName:g.name,sampleWeight:g.sampleWeight};"ERROR"===h?f.level="ERROR_LEVEL_ERROR":"WARNING"===h&&(f.level="ERROR_LEVEL_WARNNING");h={isObfuscated:!0,browserStackInfo:m};m={pageUrl:window.location.href,kvPairs:[]};Q("FEXP_EXPERIMENTS")&&(m.experimentIds=Q("FEXP_EXPERIMENTS"));if(g=g.params)for(n=u(Object.keys(g)),k=n.next();!k.done;k=n.next())k=k.value,m.kvPairs.push({key:"client."+k,value:String(g[k])});g=Q("SERVER_NAME",void 0);n=Q("SERVER_VERSION",
void 0);g&&n&&(m.kvPairs.push({key:"server.name",value:g}),m.kvPairs.push({key:"server.version",value:n}));rh("clientError",{errorMetadata:m,stackTrace:h,logMessage:f});Sf()}}if(!S("suppress_error_204_logging")){g=e;f=g.params||{};d={urlParams:{a:"logerror",t:"jserror",type:g.name,msg:g.message.substr(0,250),line:g.lineNumber,level:d,"client.name":f.name},postParams:{url:Q("PAGE_NAME",window.location.href),file:g.fileName},method:"POST"};f.version&&(d["client.version"]=f.version);if(d.postParams){g.stack&&
(d.postParams.stack=g.stack);g=u(Object.keys(f));for(h=g.next();!h.done;h=g.next())h=h.value,d.postParams["client."+h]=f[h];if(f=Q("LATEST_ECATCHER_SERVICE_TRACKING_PARAMS",void 0))for(g=u(Object.keys(f)),h=g.next();!h.done;h=g.next())h=h.value,d.postParams[h]=f[h];f=Q("SERVER_NAME",void 0);g=Q("SERVER_VERSION",void 0);f&&g&&(d.postParams["server.name"]=f,d.postParams["server.version"]=g)}of(Q("ECATCHER_REPORT_HOST","")+"/error_204",d)}Ch.add(e.message);Dh++}}}}}else throw F;}else console&&console.warn&&
console.warn("The YouTube player is not attached to the DOM. API calls should be made after the onReady event. See more: https://developers.google.com/youtube/iframe_api_reference#Events")};function Oh(a){return(0===a.search("cue")||0===a.search("load"))&&"loadModule"!==a}
